...
const base = computed<Base[]>(() => selected.value.map(c => {
  const [h, , l] = c.hsl
  return { ...c, h, l }
}))

const hue = ref(0)
const points = computed<Point[]>(() => base.value.map(b => {
    ...
    <div class="relative w-full h-[600px] rounded-md bg-white">
      <div ref="plotRef" class="absolute left-12 right-12 top-8 bottom-24" @pointerdown="onPlotDown" @wheel.passive="onWheel">
        ...
        <div class="absolute inset-0">
          <ColorPoint v-for="p in points" :key="p.name + p.hex" :color="p" :x="p.x" :y="p.y" @hover="v => hoverPoint = v" />
          ...
          <template v-if="indicator">
            <div class="absolute pointer-events-none" :style="{ left: `calc(${indicator.x}% - 0.5px)`, bottom: '-4px' }">
              <div class="w-[1px] h-2 bg-black/70"></div>
            </div>
            <div class="absolute pointer-events-none" :style="{ left: '-4px', bottom: `calc(${indicator.y}% - 0.5px)` }">
              <div class="w-2 h-[1px] bg-black/70"></div>
            </div>
          </template>
        </div>
      </div>
